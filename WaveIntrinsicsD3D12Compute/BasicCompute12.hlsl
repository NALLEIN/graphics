...
static int WPT_Row = 4, WPT_Col = 4;
...
void main(CS_INPUT input)
{
    ...
    int dst_write0 = local_x * (WPT_Row / VEC_SIZE) + ( group_x * ( TILE_N / VEC_SIZE ) ) + ( group_y * TILE_M + local_y * WPT_Col) * width1;

    // Src0 is directly used as atile.
    // It starts at the left side of src0 and walks across.
    // atile is M rows x K columns.
    int src0_read = ( group_y * TILE_M + local_y * WPT_Col) * width0;

    // int dst_write0 = local_x + ( group_x * ( TILE_N / VEC_SIZE ) ) + ( group_y * TILE_M ) * width1;
    // int src0_read = local_x + ( group_y * TILE_M ) * width0;
    // int src1_read0 = local_x + ( group_x * ( TILE_N / VEC_SIZE ) );
    int src1_read0 = (local_x * (WPT_Row / VEC_SIZE) + group_x * ( TILE_N / VEC_SIZE )  ) * width0;
    // Walk ACROSS src0 and DOWN src1:
    int w = 0;
    do
    {
		...
		for (int i = 0; i < WPT_Col; i++)
		{
            ...
            for(int j = 0; j < WPT_Row; j++) {
                ...
                dot[i][j] = mad( (float)(intel_sub_group_shuffle( arow, 2 )), (float)(intel_sub_group_shuffle( brow, 2 )), dot[i][j] );
                dot[i][j] = mad( (float)(intel_sub_group_shuffle( arow, 3 )), (float)(intel_sub_group_shuffle( brow, 3 )), dot[i][j] );
                dot[i][j] = mad( (float)(intel_sub_group_shuffle( arow, 4 )), (float)(intel_sub_group_shuffle( brow, 4 )), dot[i][j] );
                dot[i][j] = mad( (float)(intel_sub_group_shuffle( arow, 5 )), (float)(intel_sub_group_shuffle( brow, 5 )), dot[i][j] );
                dot[i][j] = mad( (float)(intel_sub_group_shuffle( arow, 6 )), (float)(intel_sub_group_shuffle( brow, 6 )), dot[i][j] );
                dot[i][j] = mad( (float)(intel_sub_group_shuffle( arow, 7 )), (float)(intel_sub_group_shuffle( brow, 7 )), dot[i][j] );
                dot[i][j] = mad( (float)(intel_sub_group_shuffle( arow, 8 )), (float)(intel_sub_group_shuffle( brow, 8 )), dot[i][j] );
                dot[i][j] = mad( (float)(intel_sub_group_shuffle( arow, 9 )), (float)(intel_sub_group_shuffle( brow, 9 )), dot[i][j] );
                dot[i][j] = mad( (float)(intel_sub_group_shuffle( arow, 10 )), (float)(intel_sub_group_shuffle( brow, 10 )), dot[i][j] );
                dot[i][j] = mad( (float)(intel_sub_group_shuffle( arow, 11 )), (float)(intel_sub_group_shuffle( brow, 11 )), dot[i][j] );
                dot[i][j] = mad( (float)(intel_sub_group_shuffle( arow, 12 )), (float)(intel_sub_group_shuffle( brow, 12 )), dot[i][j] );
                dot[i][j] = mad( (float)(intel_sub_group_shuffle( arow, 13 )), (float)(intel_sub_group_shuffle( brow, 13 )), dot[i][j] );
                dot[i][j] = mad( (float)(intel_sub_group_shuffle( arow, 14 )), (float)(intel_sub_group_shuffle( brow, 14 )), dot[i][j] );
                dot[i][j] = mad( (float)(intel_sub_group_shuffle( arow, 15 )), (float)(intel_sub_group_shuffle( brow, 15 )), dot[i][j] );
            }
        }
        src0_read += TILE_K0 / VEC_SIZE;
